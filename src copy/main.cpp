...
// disp the springs for the pendulum system
bool toggleSprings = false;

//// to turn on/off a breeze
//bool toggleBreeze = false;
    ...
    int scancode, int action, int mods)
{
    ...
    case 'S': {
        toggleSprings = !toggleSprings;
        break;
    }
    case 'B': {
        clothSystem->toggleBreeze();
        break;
    }
    default:
...
static void dispSpringsPendulum(GLProgram gl) {
    gl.disableLighting();
    gl.updateModelMatrix(Matrix4f::identity()); // update uniforms after mode change
    VertexRecorder rec;
    const Vector3f CLOTH_COLOR(0.9f, 0.9f, 0.9f);

    for (int i=0; i < pendulumSystem->particleSprings.size(); i++) {
        Vector4f spring = pendulumSystem->particleSprings[i];
        std::vector<Vector3f> s = pendulumSystem->getState();

        rec.record(s[spring[0]], CLOTH_COLOR);
        rec.record(s[spring[1]], CLOTH_COLOR);
    }
    glLineWidth(3.0f);
    rec.draw(GL_LINES);
...
static void dispSpringsCloth(GLProgram gl) {
    gl.disableLighting();
    gl.updateModelMatrix(Matrix4f::identity()); // update uniforms after mode change
    VertexRecorder rec;
    const Vector3f CLOTH_COLOR(0.9f, 0.9f, 0.9f);

    for (int i=0; i < clothSystem->particleSprings.size(); i++) {
        Vector4f spring = clothSystem->particleSprings[i];
        std::vector<Vector3f> s = clothSystem->getState();
